...
// precision設定なしのインスタンス
const mathDefault = create(all ?? {}, {
  number: 'BigNumber'
  // precisionを指定しない
});
...
const viaDefault = mathDefault.bignumber('123.456789123456789');

console.log(`直接import:       ${direct.toString()}`);
console.log(`math10経由:       ${via10.toString()}`);
console.log(`math64経由:       ${via64.toString()}`);
console.log(`mathDefault経由:  ${viaDefault.toString()}`);
console.log(`全て同じ値か:     ${direct.equals(via10) && via10.equals(via64) && via64.equals(viaDefault)}\n`);
...
const viaDefaultDiv = mathDefault.divide(mathDefault.bignumber(1), mathDefault.bignumber(3));
...
console.log(`mathDefault.divide: ${viaDefaultDiv.toString()}\n`);
...
console.log(`mathDefaultの精度設定: デフォルト（未指定）`);
console.log(`直接importしたbignumberの精度: デフォルト\n`);
...
const viaDefaultLong = mathDefault.bignumber(longDecimal);
...
console.log(`mathDefault経由:  ${viaDefaultLong.toString()}\n`);
